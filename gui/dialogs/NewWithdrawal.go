...
func NewWithdrawal(client *ent.Client) dialog.Dialog {
	...
	callback := func(confirmed bool) {
		...
		_, err = client.Transaction.Create().
			...
			SetDate(date).
			SetFromAccount(accountSelect.Get()).
	...
	return dialog.NewForm("Withdrawal", "Create", "Cancel", items, callback, window)
}
